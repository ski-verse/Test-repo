...
public static class RoadsideVisualShoulderMeshBuilder
{
    public const int DefaultSampleCount = 384;
    public const float InnerOffset = EnvironmentPlacement.RoadHalfWidth + 0.05f;
    public const float OuterOffset = EnvironmentPlacement.RoadHalfWidth + 0.58f;
    public const float SurfaceYOffset = 0.052f;
    ...
    public static Mesh CreateShoulderMesh(float side, int sampleCount = DefaultSampleCount)
    {
        ...
        var mesh = new Mesh
        {
            name = "Roadside Visual Shoulder Mesh",
    ...
    {
        var sideSign = side < 0f ? -1f : 1f;
        var lateralOffset = sideSign * (innerEdge ? InnerOffset : OuterOffset);
