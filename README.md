...
- `AdventureCharacterRollerSkierRuntimeUpdater` now prioritizes the imported humanoid prefab at `Assets/Adventure_Character/Prefabs/Man_01.prefab` as the Play Mode skier, keeps the humanoid mesh intact, constrains roller skis and boot details to the humanoid foot positions with player-root aligned rotation, constrains ski poles to the humanoid hand positions, skips the generic pole visibility pass for Adventure Character, and disables the old procedural skier animation for that path so equipment no longer floats or moves independently
- Skier Model 2.0 and `SkiClassicsSkierModelBuilder` remain as lightweight generated fallbacks if the Adventure Character asset is missing
- `SkiClassicsGameplayModelRuntimeUpdater` switches the older placeholder visual to the generated Ski Classics model before the Adventure Character humanoid pass replaces it when the imported prefab is available
...
- EditMode tests for speed, distance, HUD formatting, gradient HUD creation, camera FOV, focused player camera framing, camera look-ahead, camera shake, dramatic course path behavior, major climb profile, non-locking gradient-based climb resistance, environment clearance, visible mountain placement, rounded mountain range mesh shape, skier screen presence, realistic skier silhouette, improved skier proportions, Vasalopp-style athletic torso shape, athletic lower body stance, proper roller skier model replacement, standalone Ski Classics skier model generation, Ski Classics gameplay model swapping, Adventure Character humanoid skier configuration, connected Adventure Character rig configuration, rear-view reference image arms/legs/boots/helmet details, modern slim classic roller ski proportions, classic roller ski wheel and binding placement, refined visible dark pole proportions and gameplay-camera side placement, dedicated pole visibility runtime pass, stable double-poling animation hierarchy, Ski Classics double-poling biomechanics, stronger body-driven posture mechanics, head stability, core-driven force transfer, workout session flow, session completion buttons, stronger speed feeling runtime updates, road-edge flow cues, player input abstraction, and roller skier animation behavior
...
4. Import or keep the Adventure Character asset at `Assets/Adventure_Character/Prefabs/Man_01.prefab`.
5. Open any empty scene or create a new one.
6. Press Play.

The prototype scene is created automatically at runtime by `Assets/Scripts/SkiErgGameBootstrap.cs`, with `Assets/Scripts/AdventureCharacterRollerSkierRuntimeUpdater.cs` using the imported humanoid Adventure Character as the skier when available, `Assets/Scripts/SkiClassicsGameplayModelRuntimeUpdater.cs` and `Assets/Scripts/SkiClassicsSkierModelBuilder.cs` providing the generated fallback skier, `Assets/Scripts/PoleVisibilityRuntimeUpdater.cs` adding the dedicated high-contrast pole visibility pass, `Assets/Scripts/MountainRangeSceneUpdater.cs` replacing the placeholder mountains with Nordic low-poly mountain chains, `Assets/Scripts/SkierPresenceRuntimeUpdater.cs` increasing the skier's visual presence, `Assets/Scripts/WorkoutSessionController.cs` adding the lightweight workout session flow and finish screen, `Assets/Scripts/SpeedFeelingRuntimeUpdater.cs` improving camera composition with stronger speed-based FOV plus denser roadside and road-edge motion cues, `Assets/Scripts/GradientHudRuntimeUpdater.cs` adding the current gradient HUD line, `Assets/Scripts/SkierTechniqueRuntimeUpdater.cs` attaching pole pivots to hand references for the refined double-poling technique, and `Assets/Scripts/KeyboardPlayerInputSource.cs` providing today's keyboard controls through the input abstraction used by `Assets/Scripts/PlayerSpeedController.cs`.
